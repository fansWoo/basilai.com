...
<?=$temp['topheader']?>
		...
		<div class="textHotNews">
			<h2>熱門文章</h2>
			<?foreach($news_other_list as $value):?>
				<a href="news/view/<?=$value['newsid']?>"><?=$value['title']?></a>
			<?endforeach?>
		</div>
		<div class="newsContent">
			...
			<?foreach($news_list as $value):?>
				<div class="stage">
					<h3 class="title"><a href="news/view/<?=$value['newsid']?>"><?=$value['title']?></a></h3>
					<p class="title2"><a href="" fanScript-hrefNone class="author">Sacriley Yang</a><?=$value['updatetime']?></p>
					<?if($value['pic']):?>
					<a href="news/view/<?=$value['newsid']?>" class="pic">
						<img src="<?=$value['pic']?>">
					</a>
					<?endif?>
					<div class="message">
					...
					<p>（<a href="news/view/<?=$value['newsid']?>">繼續閱讀...</a>）</p>
				</div>
			<?endforeach?>
			<?else:?>
				<div class="stage">
					<h3 class="title"><a href="news/view/<?=$news['newsid']?>"><?=$news['title']?></a></h3>
					<p class="title2"><a href="" fanScript-hrefNone class="author">Sacriley Yang</a><?=$news['updatetime']?></p>
					<div class="message">
					<?=$news['text']?>
					</div>
					<iframe src="http://www.facebook.com/widgets/like.php?href=http://www.facebook.com/fanswoo.my&show_faces=true" scrolling="no" frameborder="0" allowTransparency="true" style="border:none;overflow:hidden;width:500px; height:65px;"></iframe>
				...
				<div class="commentList">
					...
					<div class="fb-comments" data-href="http://web.fanswoo.com/business/news/view/<?=$news['newsid']?>" data-width="520"></div>
				</div>
			<?endif?>
		</div>
<?=$temp['footer']?>